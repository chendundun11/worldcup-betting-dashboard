  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--help' || arg === '-h') {
      console.log(
        [
          'Usage:',
          '  node .\\scripts\\generate-v4-voiceover.mjs --meta <meta.json> --output <voiceover.txt> --copy <copy.txt> --style sharp',
          '  node .\\scripts\\generate-v4-voiceover.mjs --meta <meta.json> --output <voiceover.txt> --script <custom.txt>',
        ].join('\n'),
      )
      process.exit(0)
    }
...
function normalizeTotalGoalsForSpeech(value) {
  ...
  return `${text}，结合比分分布谨慎复核`
}

function trimRisk(value) {
  return String(value ?? '临场阵容、节奏和轮换还要复核')
    .trim()
...
function estimateVoiceoverSeconds(text) {
  ...
  return Number(Math.max(charCount / 6.8, 1).toFixed(1))
}
...
function buildGeneratedVoiceover(meta, style) {
  ...
  if (style === 'record') {
    return [
      `本地 AI 系统跑了一遍，${speechMatchName}，这场先记录一下。`,
      `方向暂时偏 ${mainPick}，比分参考 ${score1} 或 ${score2}。`,
      `大小球按 ${totalGoals}。`,
      `${riskNote}。`,
      '每天记录几场，只做数据记录和娱乐参考。',
    ].join('\n')
  ...
  if (style === 'explain') {
    return [
      `这场${speechMatchName}，主要看压制力和比分分布。`,
      `模型方向给到 ${mainPick}，比分先看 ${score1} 和 ${score2}。`,
      `大小球按 ${totalGoals}。`,
      `风险点在于${riskNote}。`,
      '这不是命中承诺，只是赛前数据记录和娱乐参考。',
    ].join('\n')
  }

  return [
    `这场模型方向挺明确，但我不会直接说稳。`,
    `${speechMatchName}，本地 AI 系统重新跑了一遍。`,
    `方向更偏 ${mainPick}，比分参考 ${score1} 或 ${score2}。`,
    `大小球按 ${totalGoals}。`,
    `风险点是${riskNote}。`,
    '每天记录几场，只做数据和娱乐参考。',
  ].join('\n')
...
function buildCopyText(meta, style) {
  ...
  const riskNote = meta.risk_note ?? '临场阵容、节奏和轮换还要复核'
  ...
  return [
    ...
    '',
    '【口语版】',
    `这场 ${matchName} 我让本地 AI 又跑了一遍。先看 ${mainPick}，比分盯 ${score1} 和 ${score2}，大小球看 ${totalGoals}。但 ${riskNote}，所以还是当赛前观察，每天记录几场，娱乐参考就好。`,
    '',
  ].join('\n')
}

export function writeV4VoiceoverFiles({
  copyPath,
  customScriptPath,
  metaPath,
  outputPath,
  ...
  if (voiceoverCharCount > 230) {
    warnings.push(`自定义或生成口播较长：${voiceoverCharCount} 字，可能超过 30 秒。`)
  }
...
if (isMainModule()) {
  try {
    ...
    const result = writeV4VoiceoverFiles({
