...
export function isFinishedMatchV4(match) {
...
export function getTeamMetricV4(match, side, key, fallback = 50) {
  const team = match?.[`${side}Team`]
  return clampNumber(toFiniteNumber(team?.[key] ?? match?.[`${side}${key}`], fallback), 0, 100)
}
...
export function getAttackTempoV4(match) {
  ...
  return clampNumber((homeAttack + awayAttack + (200 - homeDefense - awayDefense)) / 4, 0, 100)
}
...
export function getBestOutcomeV4(probabilities) {
...
export function mapOutcomeToMainPickV4(outcome) {
  if (outcome === 'home') return '主队胜'
  if (outcome === 'away') return '客队胜'
